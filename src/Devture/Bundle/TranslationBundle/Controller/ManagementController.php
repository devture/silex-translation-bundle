	...
	public function editAction(Request $request, $resourceId, $language) {
		...
		if ($request->isMethod('POST')) {
			...
			foreach ($modifiedResources as $modifiedResource) {
				$result = $this->getResourcePersister()->persist($sourceResource, $modifiedResource);
				if (!$result) {
					return $this->json(array(
						'ok' => false,
						'errors' => array(
							sprintf('Could not save the translation data for: %s/%s', $modifiedResource->getName(), $modifiedResource->getLocaleKey())
						),
					));
				}
			}
